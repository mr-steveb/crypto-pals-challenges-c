...
struct one_line *read_file(const char *filename) {
  ...
  { // count lines
    char *p = (char *)data;
    for (lines = 0; (p = strchr(p, '\n')); ++lines, ++p) { // count the lines for every \n
    }
  }
  { // allocate Lines buffer
    all_lines =
        (struct one_line *)malloc(sizeof(struct one_line) * (lines + 1)); // why + 1?
    if (!all_lines) {
  ...
  { // store lines
    ...
    unsigned char c = 0;
    size_t pos = 0;
    struct one_line *this_line = all_lines; // init pointer to 'start' of all_lines
    this_line->pos = data; // init point to start of data (why?)
